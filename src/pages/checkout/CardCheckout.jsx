    ...
    return (
        ...
        <div id={id} className="products flex flex-row border border-dashed border-slate-50 bg-white bg-opacity-60 shadow-lg rounded-xl overflow-hidden p-4 py-4">
            <section className="flex">
                <span className="mr-2 self-center text-red-400 hover:text-orange-800 cursor-pointer "  onClick={()=>handleDeleteDish(i,dish.id, dish.quantity)}>
                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex ml-5">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" onClick={()=>handleDecrement(dish, i, dish.id)} className="h-6 w-6 ml-1 cursor-pointer  text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </span>
                        <span className="ml-2">
                            <svg xmlns="http://www.w3.org/2000/svg" onClick={()=>handlePlus(dish, dish.id)} className="h-6 w-6 cursor-pointer text-lime-600 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
{/* tira a quantidade de pratos adicionados, ao stock */}
                ...
                <div className="flex flex-col justify-between">
                    ...
                    <p className="text-sky-900 bg-zinc-50 px-2 py-1 shadow-slate-900 text-lg shadow-lg rounded-md p-0.5">{`Price: ${getPriceValue(dish.price)*dish.quantity}`}</p> 
                </div>
        </div>
        </>
    )
